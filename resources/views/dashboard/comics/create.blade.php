...
<section class="py-12 bg-[#0d1117] min-h-screen text-[#c9d1d9]">
    <div class="max-w-3xl mx-auto px-6">
        <h1 class="text-2xl font-bold mb-4">Tambah Comic</h1>
        ...
        <form action="{{ route('dashboard.comics.store') }}" method="POST" class="space-y-4 bg-[#161b22] p-6 rounded">
            @csrf
            <div>
                <label class="block text-sm text-gray-300">Title</label>
                <input name="title" value="{{ old('title') }}" class="w-full mt-1 p-2 rounded bg-[#0b1116] border border-[#222834]" required>
            </div>

            <div>
                <label class="block text-sm text-gray-300">Author</label>
                <input name="author" value="{{ old('author') }}" class="w-full mt-1 p-2 rounded bg-[#0b1116] border border-[#222834]">
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input name="status" value="{{ old('status') }}" class="w-full mt-1 p-2 rounded bg-[#0b1116] border border-[#222834]">
                </div>
            </div>

            <div>
                <label class="block text-sm text-gray-300">Slug (optional)</label>
                <input name="slug" value="{{ old('slug') }}" class="w-full mt-1 p-2 rounded bg-[#0b1116] border border-[#222834]">
            </div>

            <div>
                <label class="block text-sm text-gray-300">Description</label>
                <textarea name="desc" class="w-full mt-1 p-2 rounded bg-[#0b1116] border border-[#222834]">{{ old('desc') }}</textarea>
            </div>
