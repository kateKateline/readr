...
<div class="max-w-2xl">
    ...
    <div class="bg-[#161b22] border border-[#21262d] rounded-lg p-6">
        <h2 class="text-xl font-semibold text-white mb-6">Edit Comment</h2>
        
        <form action="{{ route('dashboard.comments.update', $comment) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select name="comic_id" required
                        ...
                        @foreach($comics as $comic)
                            <option value="{{ $comic->id }}" {{ old('comic_id', $comment->comic_id) == $comic->id ? 'selected' : '' }}>{{ $comic->title }}</option>
                        @endforeach
                    </select>
                    @error('comic_id')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select name="parent_id"
                        ...
                        @foreach($parentComments as $parent)
                            <option value="{{ $parent->id }}" {{ old('parent_id', $comment->parent_id) == $parent->id ? 'selected' : '' }}>{{ Str::limit($parent->comment, 50) }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <textarea name="comment" rows="5" required
                              class="w-full px-4 py-2 bg-[#0d1117] border border-[#30363d] text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('comment', $comment->comment) }}</textarea>
                    @error('comment')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="is_edited" value="1" {{ old('is_edited', $comment->is_edited) ? 'checked' : '' }}
