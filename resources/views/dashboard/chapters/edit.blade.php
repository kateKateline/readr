...
<div class="max-w-2xl">
    ...
    <div class="bg-[#161b22] border border-[#21262d] rounded-lg p-6">
        <h2 class="text-xl font-semibold text-white mb-6">Edit Chapter</h2>
        
        <form action="{{ route('dashboard.chapters.update', $chapter) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="mangadex_id" value="{{ old('mangadex_id', $chapter->mangadex_id) }}" required
                           class="w-full px-4 py-2 bg-[#0d1117] border border-[#30363d] text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    @error('mangadex_id')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="chapter_number" value="{{ old('chapter_number', $chapter->chapter_number) }}" required
                           class="w-full px-4 py-2 bg-[#0d1117] border border-[#30363d] text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <div>
                    ...
                    <input type="text" name="translated_language" value="{{ old('translated_language', $chapter->translated_language) }}"
                           class="w-full px-4 py-2 bg-[#0d1117] border border-[#30363d] text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>

                <div>
                    <label class="block text-gray-300 text-sm font-medium mb-2">Pages</label>
                    <input type="number" name="pages" value="{{ old('pages', $chapter->pages) }}"
                           class="w-full px-4 py-2 bg-[#0d1117] border border-[#30363d] text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>

                <div>
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="is_unavailable" value="1" {{ old('is_unavailable', $chapter->is_unavailable) ? 'checked' : '' }}
