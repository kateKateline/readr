...
@section('page-title', 'Edit Global Chat')
...
<div class="max-w-2xl">
    ...
    <div class="bg-[#161b22] border border-[#21262d] rounded-lg p-6">
        <h2 class="text-xl font-semibold text-white mb-6">Edit Global Chat</h2>
        
        <form action="{{ route('dashboard.global-chats.update', $globalChat) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <select name="user_id" required
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('user_id', $globalChat->user_id) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                        @endforeach
                    </select>
                    @error('user_id')
                ...
                <div>
                    ...
                    <textarea name="message" rows="5" required
                              class="w-full px-4 py-2 bg-[#0d1117] border border-[#30363d] text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('message', $globalChat->message) }}</textarea>
                    @error('message')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
